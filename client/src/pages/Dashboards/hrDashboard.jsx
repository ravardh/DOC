...
import {
  FaSignOutAlt, FaUserFriends, FaUserGraduate, FaUserTie,
  FaEnvelope, FaBullhorn, FaTrash, FaEdit as FaEditIcon,
  FaCalendarAlt, FaListAlt, FaDoorOpen, FaCertificate, FaUmbrellaBeach,
} from "react-icons/fa";
import {
  DetailsModal, EditModal, OnboardingModal, RejectionModal, RemarkModal, AnnouncementModal,
...
const HRDashboard = () => {
  ...
  const [certificateForm, setCertificateForm] = useState({
    volunteer: "", certificateName: "", issuedBy: "", issueDate: "", expiryDate: "", documentUrl: "", description: "",
  });
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== "hr") { navigate("/login"); return; }
    fetchData();
  }, [navigate]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [applicantsRes, contactsRes, announcementsRes, exitRequestsRes, certificatesRes, leavesRes] =
        await Promise.all([
  ...
  const handleApplicantStatusChange = async (id, newStatus, currentStatus) => {
    try {
      ...
      fetchData();
    } catch (error) { setError("Error updating applicant status"); }
  };

  const handleLogout = async () => {
    try { await axios.post("/api/auth/logout"); } catch (error) { }
    localStorage.removeItem("user"); localStorage.removeItem("token"); navigate("/login");
  };

  const getFieldLabel = (key) => {
    const labelMap = { dob: "DATE OF BIRTH", doj: "DATE OF JOINING", dol: "DATE OF LEAVING", admittedInSchool: "ADMITTED IN SCHOOL", createdAt: "APPLIED ON", updatedAt: "UPDATED AT", __v: "VERSION", _id: "ID" };
  ...
  const handleCertificateSubmit = async () => {
    try {
      ...
      else { await axios.post("/api/hr/certificates", certificateForm); }
  ...
  if (error) return (<div className="flex items-center justify-center min-h-screen"><div className="text-red-500 text-center"><p className="text-xl font-semibold">{error}</p><button onClick={fetchData} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Retry</button></div></div>);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 gap-4 md:flex lg:flex mb-4 flex-wrap">
        {[
          ...
          { key: "leaves", icon: <FaUmbrellaBeach className="mr-2" />, label: `Leaves (${leaves.length})` },
        ].map(({ key, icon, label }) => (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {exitRequests.map((req) => (
                      <tr key={req._id}>
                        ...
                        <td className="px-6 py-4"><span className={`px-2 py-1 rounded-full text-xs font-semibold ${req.status === "approved" ? "bg-green-100 text-green-800" : req.status === "rejected" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}`}>{req.status}</span></td>
                        <td className="px-6 py-4">{req.status === "pending" && (<div className="flex space-x-2">
        ...
        {activeTab === "leaves" && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => { setLeaveForm({ volunteer: "", leaveType: "casual", startDate: "", endDate: "", reason: "" }); setShowLeaveModal(true); }} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Add Leave</button>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {leaves.map((leave) => (
                      <tr key={leave._id}>
                        <td className="px-6 py-4">{leave.volunteer?.name || "-"}</td>
                        ...
                        <td className="px-6 py-4">{leave.status === "pending" && (<div className="flex space-x-2">
                          <button onClick={() => handleLeaveUpdate(leave._id, "approved")} className="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600 text-sm">Approve</button>
                          <button onClick={() => handleLeaveUpdate(leave._id, "rejected")} className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 text-sm">Reject</button>
                        </div>)}</td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Volunteer ID *</label><input type="text" value={certificateForm.volunteer} onChange={(e) => setCertificateForm({ ...certificateForm, volunteer: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Certificate Name *</label><input type="text" value={certificateForm.certificateName} onChange={(e) => setCertificateForm({ ...certificateForm, certificateName: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Issued By *</label><input type="text" value={certificateForm.issuedBy} onChange={(e) => setCertificateForm({ ...certificateForm, issuedBy: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Issue Date *</label><input type="date" value={certificateForm.issueDate} onChange={(e) => setCertificateForm({ ...certificateForm, issueDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Expiry Date</label><input type="date" value={certificateForm.expiryDate} onChange={(e) => setCertificateForm({ ...certificateForm, expiryDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Document URL</label><input type="text" value={certificateForm.documentUrl} onChange={(e) => setCertificateForm({ ...certificateForm, documentUrl: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Description</label><textarea value={certificateForm.description} onChange={(e) => setCertificateForm({ ...certificateForm, description: e.target.value })} rows="3" className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
            </div>
      ...
      {showLeaveModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-semibold">Add Leave Request</h2>
              <button onClick={() => setShowLeaveModal(false)} className="text-gray-500 hover:text-gray-700 text-2xl">×</button>
            </div>
            <div className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Volunteer ID *</label><input type="text" value={leaveForm.volunteer} onChange={(e) => setLeaveForm({ ...leaveForm, volunteer: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Leave Type *</label>
                <select value={leaveForm.leaveType} onChange={(e) => setLeaveForm({ ...leaveForm, leaveType: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Start Date *</label><input type="date" value={leaveForm.startDate} onChange={(e) => setLeaveForm({ ...leaveForm, startDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">End Date *</label><input type="date" value={leaveForm.endDate} onChange={(e) => setLeaveForm({ ...leaveForm, endDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Reason *</label><textarea value={leaveForm.reason} onChange={(e) => setLeaveForm({ ...leaveForm, reason: e.target.value })} rows="3" className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>
            </div>
            <div className="flex justify-end space-x-4 mt-6">
              <button onClick={() => setShowLeaveModal(false)} className="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">Cancel</button>
              <button onClick={handleLeaveSubmit} className="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Add</button>
            </div>
          </div>
        </div>
      )}
